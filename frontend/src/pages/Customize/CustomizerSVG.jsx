...
//             {activeTool === "upload" && <UploadControls
//               canvasRef={canvasRef}
...
//               />
//             )}
//             {activeTool === "name" && <NameNumberInput
//               canvasRef={canvasRef}
//               updateThumbnail={() => updateThumbnail(activeIndex)}
//               saveCurrentViewState={saveCurrentViewState}
//               activeIndex={activeIndex}
...
const CustomizerSVG = () => {
  ...
  const updateAllThumbnails = () => thumbnailCanvasRefs.current.forEach((_, i) => updateThumbnail(i));
  ...
  const handleGlobalColorChange = (newColors) => {
    ...
    const updatedStates = viewStates.map((state, idx) => {
      if (!state) return null;
      if (idx === activeIndex) {
        const userObjects = extractUserObjects(canvas).map(o =>
          o.toObject(["type","left","top","scaleX","scaleY","angle","flipX","flipY","fontFamily","fill","text","src","width","height","fontSize","fontWeight","textAlign","customPart"])
      ...
      return { ...state, globalPartColors: newColors };
    });
  ...
  // Load saved view on index change
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
  ...
  return (
    <div className="customizer-page">
      ...
      <div className="customizer-container">
        ...
        <div className="customizer-main">
          <div className="vertical-toolbar">
            <VerticalToolbar onSelectTool={setActiveTool} flag={true} productType={productType} />
          </div>
          <div className="customizer-controls">
            {activeTool === "upload" && <UploadControls
              canvasRef={canvasRef}
            ...
            {activeTool === "name" && <NameNumberInput
              ...
              activeIndex={activeIndex}
            />}
            {activeTool === "export" && <ExportButtons
              canvasRef={canvasRef}
              thumbnailCanvasRefs={thumbnailCanvasRefs}
              viewStates={viewStates}
